...
from decision_transformer.dt.utils import (
    discount_cumsum,
    evaluate_on_env,
    get_d4rl_normalized_score,
    save_params,
)
from decision_transformer.pmap import (
    bcast_local_devices,
    synchronize_hosts,
    is_replicated,
)
...
def train(args):
    ...
    print(
        f"Device count: {jax.device_count()}, process count: {process_count} (id {process_id}), local device count: {local_device_count}, devices to be used count: {local_devices_to_use}"
    )
    ...
    max_eval_ep_len = args.max_eval_ep_len
    ...
    for traj in trajectories:
        ...
        traj["returns_to_go"] = jnp.array(
            discount_cumsum(traj["rewards"], 1.0) / rtg_scale
        ).reshape(-1, 1)
        traj["timesteps"] = jnp.arange(
            start=0, stop=traj_len, step=1, dtype=jnp.int32
        ).reshape(-1, 1)
    ...
    state_mean, state_std = (
        jnp.mean(state_stats, axis=0),
        jnp.std(state_stats, axis=0) + 1e-8,
    )
    ...
    for traj in trajectories:
        ...
        actions = jnp.concatenate(
            [traj["actions"], jnp.zeros((padding_len, act_dim))], axis=0
        )
        returns_to_go = jnp.concatenate(
            [traj["returns_to_go"], jnp.zeros((padding_len, 1))], axis=0
        )
        timesteps = jnp.concatenate(
            [traj["timesteps"], jnp.zeros((padding_len, 1))], axis=0
        )
        traj_mask = jnp.concatenate(
            [traj["traj_mask"], jnp.zeros((padding_len, 1))], axis=0
        )

        padding_data = jnp.concatenate(
            [states, actions, returns_to_go, timesteps, traj_mask], axis=-1
        )
        assert trans_dim == padding_data.shape[-1], padding_data.shape
        replay_buffer_data.append(padding_data)

    replay_buffer = ReplayBuffer(
        data=jnp.concatenate(replay_buffer_data, axis=0).reshape(
            local_devices_to_use, -1, max_epi_len + context_len, trans_dim
        )
    ...
    def actor_loss(
        policy_params: Any, transitions: Transition, key: jnp.ndarray
    ) -> jnp.ndarray:
        ts = transitions.ts.reshape(transitions.ts.shape[:2]).astype(
            jnp.int32
        )  # (batch_size_per_device, context_len)
        ...
        _, a_p, _ = policy_model.apply(
            policy_params, ts, s_t, a_t, rtg_t, rngs={"dropout": key}
        )

        a_t = jnp.where(
            mask.reshape(-1, 1) > 0, a_t.reshape(-1, act_dim), jnp.zeros(())
        )
        a_p = jnp.where(
            mask.reshape(-1, 1) > 0, a_p.reshape(-1, act_dim), jnp.zeros(())
        )
    ...
    def update_step(
        ...
        transitions = Transition(
            ...
            mask_t=transitions[
                :, :, state_dim + act_dim + 1 + 1 : state_dim + act_dim + 1 + 1 + 1
            ],
        )
        ...
        actor_loss, actor_grads = actor_grad(
            state.policy_params, transitions, key_actor
        )
    ...
    def sample_data(training_state, replay_buffer):
        ...
        epi_idx = jax.random.randint(
            key1,
            (int(batch_size_per_device * grad_updates_per_step),),
            minval=0,
        ...
        context_idx = jax.random.randint(
            ...
            (int(batch_size_per_device * grad_updates_per_step),),
            minval=0,
            maxval=replay_buffer.data.shape[1],
        )  # from (0, max_epi_len)
        ...
        _, transitions = jax.lax.scan(
            dynamic_slice_context, (), (transitions, context_idx)
        )
        # (num_updates_per_iter, batch_size_per_device, context_len, trans_dim)
        transitions = jnp.reshape(
            transitions, [grad_updates_per_step, -1] + list(transitions.shape[1:])
        )
    ...
    for i_train_iter in range(max_train_iters):
        ...
        eval_d4rl_score = (
            get_d4rl_normalized_score(results["eval/avg_reward"], env_name) * 100
        )
        ...
        if (
            i_train_iter % args.policy_save_iters == 0
            or i_train_iter == max_train_iters - 1
        ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--rm_normalization", action="store_true", help="Turn off input normalization"
    )
